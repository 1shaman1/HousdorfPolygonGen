...
**Произвольность направления** обеспечивает разнообразие форм впадин; **контроль метрик** — через геометрические рычаги (глубина, длина хорды на оболочке, число точек кармана) без подгонки под `area_ratio`.
...
| `area_ratio` | Отношение площадей | \(\mathrm{area}(P_0) / \mathrm{area}(P) \ge 1\) | Только **actual** в CSV (не рычаг) |
...
|--------------|-------------------|
| `one_at_a_time` | **Основной:** одна метрика по сетке, остальные uniform в `[min,max]` → кривая «ось → actual → Δ_УЛЛ» |
